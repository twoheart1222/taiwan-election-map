(()=>{
  ...
  function buildCompareUI(){
    ...
    slot.innerHTML=`<div class="archive-compare-workspace"><div class="archive-compare-top"><label class="archive-year-select"><span>A 年份</span><select id="archive-compare-a" aria-label="比較 A 年份"></select></label><button type="button" id="archive-compare-swap" class="archive-compare-swap" aria-label="交換 A 與 B 年份">⇄</button><label class="archive-year-select"><span>B 年份</span><select id="archive-compare-b" aria-label="比較 B 年份"></select></label><div class="archive-layer-field"><span>地圖圖層</span><div class="archive-segment archive-layer-switch" role="group" aria-label="比較地圖圖層"><button type="button" data-layer="winner" class="on">勝方版圖</button><button type="button" data-layer="share">得票率變化</button><button type="button" data-layer="swing">藍綠 Swing</button></div></div></div><div class="archive-compare-body"><div class="archive-compare-summary"><div class="archive-compare-heading"><div><div class="archive-compare-kicker">CROSS-ELECTION COMPARISON</div><h3 class="archive-compare-title">跨屆比較</h3></div><div class="archive-compare-note"></div></div><div class="archive-segment archive-party-switch" id="archive-party-switch" role="group" aria-label="選擇得票率變化政黨"><button type="button" data-party="DPP" class="on">民主進步黨</button><button type="button" data-party="KMT">中國國民黨</button></div><div class="archive-map-legend" id="archive-map-legend"></div><div id="archive-compare-chart" class="paired-compare-chart"></div><div class="archive-compare-stats" id="archive-compare-stats"></div></div><div class="archive-compare-inspector"><div class="compare-region-heading"><strong>選擇縣市</strong><span>顏色顯示 A → B 勝方政黨</span></div><div class="archive-flips compare-region-picker" id="archive-flips" aria-label="選擇比較縣市"></div><div class="archive-compare-detail" id="archive-compare-detail">點擊地圖查看縣市變化。</div></div></div></div>`;
  ...
  function renderLegend(rows){const legend=$('#archive-map-legend');if(!legend)return;legend.className='archive-map-legend '+compareLayer;if(compareLayer==='winner'){legend.innerHTML=`<span><i style="background:${COLORS.DPP}"></i>DPP</span><span><i style="background:${COLORS.KMT}"></i>KMT</span><span><i style="background:${COLORS.IND}"></i>其他</span><span><i style="background:#E4022B"></i>紅框＝目前選取</span>`;return}if(compareLayer==='share'){legend.innerHTML=`<span><i style="background:${partyColor(compareParty)};opacity:.35"></i>顏色越深＝該年度 ${PARTY_LABELS[compareParty]} 得票率越高</span>`;return}const max=absoluteDomain(rows,'swing');legend.innerHTML=`<span>KMT −${max.toFixed(1)}pp</span><i class="legend-bar"></i><span>DPP +${max.toFixed(1)}pp</span>`}
  function renderCompareSummary(){
    ...
    $('#archive-flips').innerHTML=rows.map(r=>`<button type="button" class="archive-flip${r.flip?' changed':''}" data-county="${esc(r.name)}" aria-label="${esc(r.name)}：${compareA} ${esc(r.pA)} 到 ${compareB} ${esc(r.pB)}"><i style="background:${partyColor(r.pA)}"></i><span>${esc(r.name)}</span><b>→</b><i style="background:${partyColor(r.pB)}"></i></button>`).join('');
