(()=>{
  ...
  async function renderCompare(){
    ...
    try{
      ...
      $('#local-flips').innerHTML=rows.map(r=>`<button type="button" class="local-flip-chip${r.flip?' changed':''}" data-county="${esc(r.name)}"><i style="background:${partyColor(r.aParty)}"></i><span>${esc(r.name)}</span><b>→</b><i style="background:${partyColor(r.bParty)}"></i></button>`).join('');
